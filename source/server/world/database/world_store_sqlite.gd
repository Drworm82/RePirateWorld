...
func save_boat(boat: Dictionary) -> void:
	db.query_with_bindings(
		"UPDATE boats SET tier=?, state=?, instance_name=?, x=?, y=?, heading=?, destination_instance=?, target_x=?, target_y=?, departure_ms=?, eta_ms=?, route_start_x=?, route_start_y=? WHERE boat_id=? AND owner_player_id=?;",
		[
			...
			float(boat.get("route_start_x", 0.0)), float(boat.get("route_start_y", 0.0)),
